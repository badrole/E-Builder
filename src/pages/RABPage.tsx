...
export default function RABPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 md:px-6 lg:px-8 py-8 space-y-8 overflow-hidden">
      ...
      {step === 1 && (
        <div className="bg-white rounded-2xl border border-outline-variant p-6 sm:p-8 space-y-6">
          ...
          <div className="flex flex-wrap gap-2 items-end"><input placeholder="Nama item" value={newItem.name} onChange={e => setNewItem({...newItem, name: e.target.value})} className="input-field flex-1 min-w-[120px]" /><input placeholder="Satuan" value={newItem.unit} onChange={e => setNewItem({...newItem, unit: e.target.value})} className="input-field w-full max-w-20" /><input type="number" placeholder="Qty" value={newItem.qty || ''} onChange={e => setNewItem({...newItem, qty: Number(e.target.value)})} className="input-field w-full max-w-20" /><input type="number" placeholder="Harga" value={newItem.price || ''} onChange={e => setNewItem({...newItem, price: Number(e.target.value)})} className="input-field w-full max-w-32" /><button onClick={addItem} className="btn-primary w-full sm:w-auto">+ Tambah</button></div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-2xl border border-outline-variant p-6 sm:p-8 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="p-4 bg-primary/5 rounded-xl text-center overflow-hidden"><p className="text-caption text-outline font-semibold uppercase">Total Estimasi</p><p className="text-2xl md:text-3xl lg:text-4xl font-black text-primary price-text-strong">{formatRupiah(total)}</p></div>
            ...
            <div className="space-y-2 text-sm"><div className="flex justify-between gap-3 py-2 border-b border-outline-variant/30"><span>Material</span><span className="font-semibold price-text-strong text-right">{formatRupiah(materialTotal)}</span></div><div className="flex justify-between gap-3 py-2 border-b border-outline-variant/30"><span>Upah Tenaga Kerja ({project.area}m² × Rp150.000)</span><span className="font-semibold price-text-strong text-right">{formatRupiah(laborCost)}</span></div><div className="flex justify-between gap-3 py-2 border-b border-outline-variant/30"><span>Overhead (10%)</span><span className="font-semibold price-text-strong text-right">{formatRupiah(Math.round(overhead))}</span></div><div className="flex justify-between gap-3 py-2 border-b border-outline-variant/30"><span>Multiplier Kualitas</span><span className="font-semibold">×{multiplier}</span></div><div className="flex justify-between gap-3 py-2 font-bold text-lg"><span>TOTAL</span><span className="text-primary price-text-strong text-right">{formatRupiah(total)}</span></div></div>
          </div>
