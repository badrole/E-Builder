...
import { fetchPartners, mapPartnerToConstructPackage } from '../lib/partners';

export default function EConstructPage() {
  const [contractors, setContractors] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [fetchError, setFetchError] = useState('');
  const packages = contractors.length > 0 ? contractors : constructPackages;

  useEffect(() => {
    fetchPartners('Contractor', 'active')
      .then(data => setContractors(data.map(mapPartnerToConstructPackage)))
      .catch(error => {
        console.error('Supabase contractor fetch failed:', error);
  ...
  return (
    <div className="max-w-container mx-auto px-4 sm:px-8 py-8 sm:py-12 space-y-12">
      ...
      {loading && <div className="bg-white rounded-2xl border border-outline-variant p-6 text-on-surface-variant">Memuat mitra konstruksi dari Supabase...</div>}
      {fetchError && <div className="bg-amber-50 text-amber-800 rounded-2xl border border-amber-200 p-4 text-sm">{fetchError}</div>}
      {!loading && packages.length === 0 && <div className="bg-white rounded-2xl border border-outline-variant p-6 text-on-surface-variant">Belum ada mitra konstruksi.</div>}
        ...
        {packages.map((pkg, i) => (
          <div key={pkg.id} className={`bg-white rounded-2xl border-2 overflow-hidden flex flex-col transition-all hover:shadow-xl ${i === 2 ? 'border-cta-amber shadow-lg scale-[1.02]' : 'border-outline-variant shadow-card'}`}>
            ...
            <div className="p-6 flex-1 flex flex-col space-y-4">
              ...
              <div><h4 className="font-semibold text-sm mb-2">Tim:</h4><div className="flex flex-wrap gap-1">{pkg.team.map((t: string) => <span key={t} className="chip text-xs">{t}</span>)}</div></div>
              <div><h4 className="font-semibold text-sm mb-2 text-green-700">✓ Termasuk:</h4><ul className="space-y-1">{pkg.included.map((i: string) => <li key={i} className="text-xs text-on-surface-variant flex items-center gap-1"><span className="material-symbols-outlined text-green-500 text-sm">check</span>{i}</li>)}</ul></div>
