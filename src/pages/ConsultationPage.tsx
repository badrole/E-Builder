...
import { fetchPartners, mapPartnerToExpert } from '../lib/partners';

const specFilters = ['Semua', 'Arsitek', 'Interior Designer', 'Civil Engineer', 'Konsultan RAB'];

export default function ConsultationPage() {
  const [filter, setFilter] = useState('Semua');
  const [supabaseExperts, setSupabaseExperts] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [fetchError, setFetchError] = useState('');
  const allExperts = supabaseExperts.length > 0 ? supabaseExperts : experts;
  const filtered = filter === 'Semua' ? allExperts : allExperts.filter(e => e.spec === filter);
  ...
  useEffect(() => {
    fetchPartners('Expert', 'active')
      .then(data => setSupabaseExperts(data.map(mapPartnerToExpert)))
      .catch(error => {
        console.error('Supabase expert fetch failed:', error);
        setFetchError('Data ahli belum dapat dimuat. Menampilkan data lokal.');
      })
  ...
  return (
    <div className="max-w-container mx-auto px-4 sm:px-8 py-8 space-y-10">
      ...
      {loading && <div className="bg-white rounded-2xl border border-outline-variant p-6 text-on-surface-variant">Memuat ahli dari Supabase...</div>}
      {fetchError && <div className="bg-amber-50 text-amber-800 rounded-2xl border border-amber-200 p-4 text-sm">{fetchError}</div>}
      {!loading && filtered.length === 0 && <div className="bg-white rounded-2xl border border-outline-variant p-6 text-on-surface-variant">Belum ada ahli yang sesuai.</div>}
      {/* Experts */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">{filtered.map(e => (
        <div key={e.id} className="bg-white rounded-xl shadow-card border-t-4 border-royal-blue overflow-hidden flex flex-col hover:shadow-card-hover transition-all">
          <div className="p-6 flex-1 space-y-3">
            ...
            <div className="flex flex-wrap gap-1">{e.topics.slice(0, 3).map((t: string) => <span key={t} className="text-[10px] bg-chip-bg px-2 py-0.5 rounded-full font-semibold">{t}</span>)}</div>
          </div>
