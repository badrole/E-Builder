  ...
  return (
    <div className="max-w-container mx-auto px-4 md:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl border border-outline-variant p-4 overflow-hidden"><p className="text-caption text-outline">Bulan Ini</p><p className="text-2xl font-black text-primary price-text">Rp15,5jt</p></div>
        <div className="bg-white rounded-2xl border border-outline-variant p-4 overflow-hidden"><p className="text-caption text-outline">Total Pendapatan</p><p className="text-2xl font-black price-text">Rp89jt</p></div>
        <div className="bg-white rounded-2xl border border-outline-variant p-4 overflow-hidden"><p className="text-caption text-outline">Pekerjaan</p><p className="text-2xl font-black break-words">{mockEarnings.jobs}</p></div>
        <div className="bg-white rounded-2xl border border-outline-variant p-4 overflow-hidden"><p className="text-caption text-outline">Rating</p><p className="text-2xl font-black flex items-center gap-1"><span className="material-symbols-outlined text-warm-amber material-icon-filled">star</span>{mockEarnings.rating}</p></div>
      </div>
      <section className="bg-white rounded-2xl border border-outline-variant p-6"><h2 className="font-bold text-lg mb-4">Permintaan Pekerjaan</h2>
        {mockJobs.map(j => (<div key={j.id} className="flex flex-wrap items-center justify-between py-4 border-b border-outline-variant/30 last:border-0 gap-3">
          <div className="min-w-0"><h4 className="font-semibold break-words">{j.category}</h4><p className="text-caption text-outline break-words">{j.client} • {j.location} • {j.date}</p><p className="text-primary font-bold text-sm price-text">{j.budget}</p></div>
