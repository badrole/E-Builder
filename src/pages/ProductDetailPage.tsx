    ...
    <div className="max-w-container mx-auto px-4 md:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="p-4 md:p-6 bg-primary/5 rounded-2xl overflow-hidden"><p className="text-2xl md:text-4xl lg:text-5xl font-black text-primary price-text-strong"><PriceCountUp value={p.price} /></p><p className="text-on-surface-variant">per {p.unit}</p></div>
      ...
      {related.length > 0 && <section><h2 className="text-h3 font-bold mb-4">Produk Terkait</h2><div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">{related.map(r => (<Link key={r.id} to={`/materials/product/${r.id}`} className="bg-white rounded-2xl border border-outline-variant p-3 hover:shadow-md overflow-hidden"><div className="aspect-square bg-surface-container rounded-xl overflow-hidden mb-2"><img className="w-full h-full object-cover" src={r.image} alt={r.name} /></div><h4 className="font-bold text-sm line-clamp-2 break-words">{r.name}</h4><p className="text-primary font-bold text-sm mt-1 price-text">{formatRupiah(r.price)}</p></Link>))}</div></section>}
    </div>
  );
}
