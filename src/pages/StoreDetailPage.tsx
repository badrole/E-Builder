...
export default function StoreDetailPage() {
  ...
  return (
    <div className="max-w-container mx-auto px-4 md:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">{storeProducts.map(p => (
        <div key={p.id} className="bg-white rounded-2xl border border-outline-variant p-3 hover:shadow-md transition-all overflow-hidden"><Link to={`/materials/product/${p.id}`}><div className="aspect-square bg-surface-container rounded-xl overflow-hidden mb-3"><img className="w-full h-full object-cover" src={p.image} alt={p.name} /></div><h4 className="font-bold text-sm line-clamp-2 break-words">{p.name}</h4></Link><p className="text-primary font-bold mt-2 price-text">{formatRupiah(p.price)}/{p.unit}</p><button onClick={() => addToCart({productId:p.id,name:p.name,price:p.price,quantity:1,unit:p.unit,image:p.image,storeName:p.storeName})} className="w-full bg-royal-blue text-white py-2 rounded-lg text-xs font-semibold mt-2">+ Keranjang</button></div>
      ))}</div>
    </div>
  );
}
