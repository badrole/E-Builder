  ...
  return (
    <div className="max-w-container mx-auto px-4 md:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">{filtered.filter(t => t.available).map(t => (
        <div key={t.id} className="bg-white rounded-2xl border border-outline-variant p-4 space-y-3 overflow-hidden"><div className="flex items-center gap-3"><img className="w-12 h-12 rounded-full object-cover flex-shrink-0" src={t.image} alt={t.name} /><div className="min-w-0"><h4 className="font-bold break-words">{t.name}</h4><p className="text-caption text-outline break-words">{t.category}</p></div></div><div className="flex justify-between gap-3 text-sm"><span>⏱ Estimasi tiba: {t.arrival}</span><span className="flex items-center gap-1"><span className="material-symbols-outlined text-warm-amber text-sm material-icon-filled">star</span>{t.rating}</span></div><div className="flex flex-wrap justify-between items-center gap-2"><span className="font-bold text-primary price-text">{formatRupiah(t.fee)}</span><button onClick={() => handleSubmit(t.id)} className="bg-error text-white px-4 py-2 rounded-lg text-sm font-bold">Panggil Sekarang</button></div></div>
      ))}</div>
    </div>
  );
}
