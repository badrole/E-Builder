        ...
        {filtered.map(w => (
          <div key={w.id} className="bg-white rounded-2xl border border-outline-variant/30 shadow-sm overflow-hidden hover:shadow-md transition-all">
            ...
            <div className="p-5 space-y-3"><h4 className="font-bold">{w.name}</h4><p className="text-caption text-outline">{w.spec} • {w.city}</p>
              <div className="flex flex-wrap items-center justify-between gap-2"><span className="font-bold text-primary price-text">{formatRupiah(w.price)}{w.priceUnit}</span><Link to={`/booking/${w.id}`} className="bg-royal-blue text-white px-4 py-2 rounded-lg text-sm font-semibold">Pesan</Link></div>
            </div>
          </div>
        ))}
