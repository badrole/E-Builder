    ...
    <div className="max-w-container mx-auto px-4 md:px-6 lg:px-8 py-8 space-y-8 overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="relative h-64 rounded-2xl overflow-hidden"><img className="w-full h-full object-cover" src={pkg.image} alt={pkg.name} /><div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent"></div><h1 className="absolute bottom-6 left-6 right-6 text-white text-h2 font-bold break-words">{pkg.name}</h1></div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-2xl border border-outline-variant p-6 sticky top-24 space-y-4">
            <div className="p-4 bg-primary/5 rounded-xl overflow-hidden"><p className="text-sm text-outline font-semibold">Mulai dari</p><p className="text-2xl md:text-4xl lg:text-5xl font-black text-primary price-text-strong"><PriceCountUp value={pkg.price} /></p><p className="text-sm text-on-surface-variant">{pkg.duration}</p></div>
