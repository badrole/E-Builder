...
import { fetchPartnerById, insertBooking, mapPartnerToExpert } from '../lib/partners';
import { isValidIndonesianWhatsApp } from '../utils/formatters';

export default function ConsultationBookPage() {
  const { id } = useParams();
  const mockExpert = experts.find(e => e.id === id);
  const [supabaseExpert, setSupabaseExpert] = useState<any | null>(null);
  const [loading, setLoading] = useState(!mockExpert);
  const e = supabaseExpert || mockExpert || experts[0];
  const navigate = useNavigate();
  const { addConsultation } = useStore();
  const [form, setForm] = useState({ customerName: '', topic: e.topics[0], date: '', time: '09:00', description: '', phone: '', paymentMethod: 'transfer' });
  const [error, setError] = useState('');

  useEffect(() => {
    if (!id || mockExpert) return;
    fetchPartnerById(id)
      .then(partner => {
        if (partner?.type === 'Expert' && partner.status === 'active') {
          const mapped = mapPartnerToExpert(partner);
          setSupabaseExpert(mapped);
          setForm(current => ({ ...current, topic: mapped.topics[0] }));
        }
      })
      .catch(error => console.error('Supabase consultation partner fetch failed:', error))
  ...
  const handleSubmit = async (ev: React.FormEvent) => {
    ...
    if (!isValidIndonesianWhatsApp(form.phone)) return setError('Nomor WhatsApp harus nomor Indonesia yang valid.');
    try {
      if (supabaseExpert?.supabaseId) {
        await insertBooking({
          customer_name: form.customerName,
          customer_phone: form.phone,
          service_type: form.topic,
          partner_id: supabaseExpert.supabaseId,
          booking_date: `${form.date} ${form.time}`,
          address: 'Konsultasi online',
      ...
      console.error('Supabase consultation booking insert failed:', error);
      setError('Booking belum berhasil dikirim. Silakan coba lagi.');
    }
  };

  if (loading) return <div className="max-w-2xl mx-auto px-4 sm:px-8 py-16 text-on-surface-variant">Memuat data ahli...</div>;
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-8 py-8 space-y-6">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-outline-variant p-6 sm:p-8 space-y-4">
        ...
        <div><label className="text-sm font-semibold block mb-1">Nama Pelanggan</label><input required value={form.customerName} onChange={ev => setForm({...form, customerName: ev.target.value})} className="input-field" placeholder="Nama lengkap" /></div>
        <div><label className="text-sm font-semibold block mb-1">Topik</label><select value={form.topic} onChange={ev => setForm({...form, topic: ev.target.value})} className="input-field">{e.topics.map((t: string) => <option key={t}>{t}</option>)}</select></div>
